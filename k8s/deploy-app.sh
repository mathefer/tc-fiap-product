...
# Deploy tc-fiap-product Application to EKS
...
deploy_app() {
    print_header "Deploying tc-fiap-product Application"
...
wait_for_app() {
    ...
    kubectl wait --for=condition=ready pod -l app=product-app --timeout=300s
...
show_status() {
    ...
    echo "Pods:"
    kubectl get pods -l app=product-app
    ...
    echo ""
    echo "HPA:"
    kubectl get hpa product-app-hpa

    # Get external URL
    EXTERNAL_IP=$(kubectl get service product-service -o jsonpath='{.status.loadBalancer.ingress[0].hostname}')
    if [ -n "$EXTERNAL_IP" ]; then
        echo ""
        print_status "Application is accessible at: http://${EXTERNAL_IP}:8081"
        print_status "Swagger documentation: http://${EXTERNAL_IP}:8081/swagger/index.html"
    fi
}

# Main execution
main() {
    print_header "Starting tc-fiap-product Application Deployment"
    ...
    print_status "tc-fiap-product application deployment completed successfully!"
}
